...
A lightweight two-step workflow for fixing a known bug. No RFC, no initiative plan — just a description, a spec, and the implementation.

```txt
Bug found
     │
     ▼ Step 1
  Fill hotfix description    (~/.claude/templates/hotfix-template.md)
     │
     ▼ Step 2
/1-hotfix-plan-task    →    Hotfix tech spec
     │
     ▼ Step 3
/2-hotfix-execute-plan    →    Implemented fix
```
...
**Template:** `~/.claude/templates/hotfix-template.md`

Copy the template into your repo and fill it in:

```bash
cp ~/.claude/templates/hotfix-template.md \
   .claude/hotfixes/<hotfix-slug>/description.md
```

Fill the YAML frontmatter (`title`, `severity`, `affected-versions`, `environments`, `tech-stack`, `context-paths`) and the required sections:

- **Symptom** — what users or systems are observing (required)
- **Root cause** — your hypothesis, if you have one (optional; leave blank and Claude will investigate)
- **Fix approach** — your preferred fix strategy, if you have one (optional; leave blank and Claude will propose)
- **Verification steps** — how you'll confirm the fix works (required)
- **Scope** and **Known risks** — optional but recommended

---

### Step 2 — Generate the tech spec

**Command:** `~/.claude/commands/1-hotfix-plan-task.md`

```text
/1-hotfix-plan-task .claude/hotfixes/<hotfix-slug>/description.md \
                    --output .claude/hotfixes/<hotfix-slug>/spec.md
```

Reads the description, loads any `context-paths`, infers a root cause and fix approach if either was left blank, and writes a tech spec covering root cause analysis, technical scope, implementation details, verification steps, and risks.
...
**Command:** `~/.claude/commands/2-hotfix-execute-plan.md`

```text
/2-hotfix-execute-plan .claude/hotfixes/<hotfix-slug>/spec.md [--version 1.4.3]
```

Reads the spec, explores the codebase, implements the changes surgically (no surrounding refactors), runs tests, executes the manual verification steps, and optionally updates `CHANGELOG.md` if `--version` is provided.
...
└── hotfixes/
    └── <hotfix-slug>/
        ├── description.md    ← filled by hand (Step 1)
        └── spec.md           ← generated by /1-hotfix-plan-task (Step 2)
```
...
**Scenario:** Users are getting a 500 error on checkout since v1.4.2. You suspect the tax calculation module but aren't sure of the exact cause.

```bash
# Step 1 — copy template and fill it in
cp ~/.claude/templates/hotfix-template.md \
   .claude/hotfixes/checkout-500-on-submit/description.md
# → fill Symptom, Verification steps; leave Root cause and Fix approach blank

# Step 2 — generate tech spec (Claude infers root cause and fix)
/1-hotfix-plan-task .claude/hotfixes/checkout-500-on-submit/description.md \
                    --output .claude/hotfixes/checkout-500-on-submit/spec.md

# Step 3 — implement the fix and bump the patch version
/2-hotfix-execute-plan .claude/hotfixes/checkout-500-on-submit/spec.md \
                       --version 1.4.3
```
